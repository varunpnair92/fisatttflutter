...
class LabAllotmentPage extends StatelessWidget {
 ...
 List<Widget> hourCells = List.generate(hours.length, (index) {
  ...
  // Map hour 8 to LB in the entry's hours string
  final entryHours = dayEntries.map((entry) {
    return entry['hours']?.replaceAll('8', 'LB');
  }).toList();
  
  final hourNumber = hour == 'LB' ? 'LB' : hour.replaceFirst('H', '');
  final matchingEntries = dayEntries.where((entry) {
    final modifiedHours = entry['hours']?.split(',').map((e) => e.trim()).toList();
    return modifiedHours?.contains(hourNumber) ?? false;
  }).toList();
